class Interview < ActiveRecord::Base
  ...
  # NOTE: Currently we think 'recruiter' is 'owner', here might be changed if someday requirement is different.
  scope :owned_by, ->(user_id) { includes(:opening_candidate => [:opening]).where('openings.recruiter_id = ?', user_id) }
